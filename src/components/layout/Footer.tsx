...
import { Zap, MapPin, Phone, Mail, Globe, MessageSquare } from 'lucide-react'
    ...
    <footer className="bg-dark text-white">
      <div className="container-xl py-16 lg:py-20">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="font-display font-bold text-xl tracking-tight">
                Metro<span className="text-secondary">Blue</span>
              </span>
            </div>
            <p className="text-white/50 text-sm leading-relaxed mb-4">
              Nigeria's trusted technology partner — building world-class software, web solutions, and IT training since 2019.
            </p>
            <p className="text-white/30 text-xs font-mono">RC-1634086 · Lagos, Nigeria</p>
          </div>
          ...
          {/* Solutions */}
          <div>
            <h4 className="font-display font-bold text-white mb-5">Solutions</h4>
            <ul className="space-y-3">
              {solutionLinks.map(({ label, sub }) => (
                <li key={label}>
                  <button onClick={() => scrollTo('solutions')} className="text-left group">
                    <span className="text-white/50 text-sm group-hover:text-secondary transition-colors block">
                      {label}
                    </span>
                    <span className="text-white/25 text-xs">{sub}</span>
                  </button>
                </li>
              ))}
            </ul>
      ...
      <div className="border-t border-white/10">
        <div className="container-xl py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/30 text-xs text-center sm:text-left">
            © {new Date().getFullYear()} MetroBlue Tech Systems Limited. RC-1634086. All rights reserved.
          </p>
          <p className="text-white/20 text-xs">Built in Lagos, Nigeria 🇳🇬</p>
        </div>
      </div>
    </footer>
  )
